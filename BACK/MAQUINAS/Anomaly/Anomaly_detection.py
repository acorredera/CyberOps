import os,sys
sys.path.append(os.path.dirname(os.path.dirname(os.path.realpath(__file__))).rsplit("/",1)[0])
print (os.path.join(os.path.dirname(__file__)))
print (os.path.dirname(os.path.dirname(os.path.realpath(__file__))).rsplit("/",1)[0])
...
PATH=os.path.join(os.path.dirname(os.path.dirname(os.path.realpath(__file__))).rsplit("/",1)[0],"MAQUINAS", "Anomaly")
# import model_params
...
servers = ["gamora", "fury", "nebula", "groot", "hulk"]
dict= {"gamora":{"room_id":1, "rack_id":1, "server_id":"gamora.lsi.die"}, #10.40.39.30
       ...
       "hulk": {"room_id":2, "rack_id":1, "server_id":"hulk"},
       "cris-X550VXK": {"room_id":3, "rack_id":1, "server_id":"cris-X550VXK"}} #10.40.39.33
...
while(True):
    ...
    for metric_type in metrics:
        if metric_type=="utilization" or metric_type=="temperature": #temperature
            base_file = (PATH + "/Data_Center/%s_train.csv") %metric_type
            aux_file  = (PATH + "/Data_Center/%s_aux.csv") %metric_type
