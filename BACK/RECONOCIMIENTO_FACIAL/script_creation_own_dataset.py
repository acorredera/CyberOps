...
for emotion in emotion_list:
    ...
    for i in range(0,11):
        ...
        if flag == 1:
            ...
            cv2.imwrite((PATH+"/BANCO_IMAGENES/IMAGENES_PROPIAS/%s/" %emotion) + filename, image)
            camera.release()
...
def detect_faces(emotion):
    files = glob.glob(PATH+"/BANCO_IMAGENES/IMAGENES_PROPIAS/%s/*" % emotion)  # Get list of all images with emotion
    print(files)
    ...
    for f in files:
        ...
        for (x, y, w, h) in facefeatures:  # get coordinates and size of rectangle containing face
            ...
            try:
                ...
                cv2.imwrite(PATH+"/BANCO_IMAGENES/IMAGENES_PROPIAS/CascadeClassifier/%s/%s.jpg" %(emotion, filenumber), out)  # Write image
            except:
