...
def get_files(emotion):  # Define function to get file list, randomly shuffle it and split 80/20
    ...
    training = files[:int(len(files) * 0.95)]  # get first 80% of file list
...
for i in range(0, 1): #range(0,10)
    ...
    clf6.fit(npar_train, training_labels)

    print("getting accuracies %s" % i)  # Use score() function to get accuracy
    npar_pred = np.array(prediction_data)
    pred_lin = clf6.score(npar_pred, prediction_labels)
...
joblib.dump(clf6, PATH+'/BC_own_dataset_Classifier_py35.pkl') #Save classifier