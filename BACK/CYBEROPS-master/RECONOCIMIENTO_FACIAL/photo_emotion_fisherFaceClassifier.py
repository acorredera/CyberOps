    ...
    if flag==1:
        ...
        for (x, y, w, h) in facefeatures:  # get coordinates and size of rectangle containing face
            ...
            try:
                ...
                cv2.imwrite(PATH+'/Webcam_photos/Classifier/' + filename, out) #(picture[19:]), out)  # Write image (picture[19:])
            except:
        ...
        fishface.read(PATH+"/MODELOS/own_fisher_face_model_py35.xml") # LOADING THE FISHER_RECOG ALREADY TRAINED AND SAVED (see face_emotion_recognition_classifier.py)
        ...
        with open(PATH+'/Webcam_photos/emotions.csv', 'a') as csvfile:
        ...
        for (x, y, w, h) in facefeatures:  # get coordinates and size of rectangle containing face
            ...
            try:
                out = cv2.resize(gray, (350, 350))  # Resize face so all images have same size
                cv2.imwrite(PATH+'/Webcam_photos/Classifier/' + filename, out)  # Write image
            except:
                pass  # If error, pass file
        ...
        fishface = cv2.face.FisherFaceRecognizer_create() #fishface = cv2.createFisherFaceRecognizer() -> python 2  # Initialize fisher face classifier
        fishface.read(PATH+"/MODELOS/own_fisher_face_model_py35.xml")
