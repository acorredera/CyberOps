...
if __name__ == "__main__":
    ...
    PATH_mirtoolbox = os.path.join(PATH_ini, "AAdata"+settings.operative_system+"libs"+settings.operative_system+"MIRtoolbox1.7")
    ...
    # LOAD MODEL
    path_model = os.path.join(PATH, "AAdata"+settings.operative_system+"models"+settings.operative_system+"cnn_arousal"+settings.operative_system+"Recola_audio_CNN_FFT_lastLayers_TranfLearn_arousal_option_1_optimizer_rms_lr_0.0001_lossf_mse_batch_32_norm_7_codeChannels_3_EpochFinalModel_10")
    model = loader.loadModelKeras(weights_path=path_model + ".h5", json_path=path_model + ".json")
    #Parameters:
    root_folder = os.path.join(PATH,"AAdata"+settings.operative_system+"auto_generated_data"+settings.operative_system)
    # - spectrum generation parameters
    fs = 16000.0
    N = 1343.0
    fsize = 0.1
    fstep = 0.04
    output_folder_spectrum = os.path.join(root_folder, "audio_spectrums"+settings.operative_system)
    ...
    output_imgs_folder = os.path.join(root_folder, "audio_images"+settings.operative_system)
    output_img_folder_final = os.path.join(root_folder, "audio_images_final"+settings.operative_system)
    ...
    aux_audio_folder = os.path.join(PATH, "AAdata"+settings.operative_system+"auto_generated_data"+settings.operative_system)
    print("aux_folder:", aux_audio_folder)
    #NOISE PARAMETERS:
    ...
    if(not(os.path.exists(aux_audio_folder))):
        os.makedirs(aux_audio_folder+ "audio"+settings.operative_system)
        os.makedirs(aux_audio_folder + "audio_functionals"+settings.operative_system)
        os.makedirs(aux_audio_folder + "audio_images"+settings.operative_system)
        os.makedirs(aux_audio_folder + "audio_images_final"+settings.operative_system)
        os.makedirs(aux_audio_folder + "audio_spectrums"+settings.operative_system)
    else:
    ...
    WAVE_OUTPUT_FILENAME =os.path.join(PATH, "AAdata"+settings.operative_system+"auto_generated_data"+settings.operative_system+"audio"+settings.operative_system)
    ...
    while(True):
        ...
        shutil.rmtree(aux_audio_folder + "audio_functionals"+settings.operative_system)
        shutil.rmtree(aux_audio_folder + "audio_images"+settings.operative_system)
        shutil.rmtree(aux_audio_folder + "audio_images_final"+settings.operative_system)
        shutil.rmtree(aux_audio_folder + "audio_spectrums"+settings.operative_system)
