...
export class DetallesServidoresPage {
  ...
  ngAfterContentInit() {
    ...
    this.timeScale = d3.scaleLinear()
      .domain([0, this.numData - 1])
      .range([0.025 * this.pi2, 0.975 * this.pi2]);
...
d3.csv("../../assets/SciAm_minute_per_day_2014.csv").then((babyData) => {
  ...
  // babyData.forEach(d => {
  //   console.log(d);
  //     d.time =(parseInt(d.time)+parseInt(d.time)).toString()
  //     d.births = (parseInt(d.births)+parseInt(d.births)).toString()
  //     d.line = (parseInt(d.line)+parseInt(d.line)).toString()
  ...
  this.gridLines.selectAll(".axis-line")
      ...
      // .attr("d", d => arcPath(this.birthScale(d)))
  ...
  //     .accessors({
  //         x: d => this.birthScale(d.births) * Math.cos(this.timeScale(d.time) - this.pi1_2),
